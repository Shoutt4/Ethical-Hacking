    ...
    const tick = () => {
      ...
      if (trackRef.current) {
        trackRef.current.style.transform =
          `translate3d(${-(BUFFER * W) - frac * W}px,0,0)`
      }

      if (!reducedRef.current) {
        slidesRef.current.forEach((el, k) => {
          const layer =
            el.querySelector<HTMLElement>('[data-parallax]')

          if (layer) {
            const disp = (k - frac) * W

            layer.style.transform =
              `translateX(${(-disp * PARALLAX).toFixed(1)}px) scale(1.12)`
          }
        })
      }
        ...
        <div className="mx-auto w-full max-w-7xl px-5 md:px-8">
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-[1fr_280px] xl:gap-12">
            <div
              ...
              tabIndex={0}
              onKeyDown={onKeyDown}
              className="overflow-hidden border border-edge/70 bg-coal/40 outline-none select-none focus-visible:outline-2 focus-visible:outline-volt"
            >
              <div
                ref={trackRef}
                ...
                {offsets.map((k) => {
                  ...
                  return (
                    <div
                      ...
                      ref={(el) => {
                        if (el) {
                          slidesRef.current.set(k, el)
                        } else {
                          slidesRef.current.delete(k)
                        }
                      }}
            ...
            <nav
              aria-label="Índice de productos"
