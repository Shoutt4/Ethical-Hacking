      ...
      <div className="mx-auto w-full max-w-6xl px-5 md:px-8">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {COURSES.map((course, i) => (
            <Reveal key={course.code} className={i === COURSES.length - 1 ? 'md:col-span-1' : ''}>
              <article
                className={`group flex h-full flex-col border border-edge p-7 transition-colors duration-200 hover:border-volt ${
                  i === COURSES.length - 1 ? 'md:border-dashed' : ''
                }`}
              >
                <div className="mb-6 flex items-baseline gap-3 font-code">
                  <span className="text-[44px] leading-none font-medium text-white">{course.day}</span>
                  <span className="text-sm tracking-[3px] text-volt">{course.month}</span>
                </div>
                <h3 className="mb-2 font-display text-xl leading-snug tracking-wide text-white uppercase">
                  {course.title}
                </h3>
                ...
                <a
                  href="#contacto"
                  className="mt-6 inline-flex items-center justify-center gap-2.5 border border-edge-strong px-4 py-3 text-xs font-bold tracking-[1.5px] text-white uppercase no-underline transition-colors duration-150 hover:border-volt hover:text-volt-light"
                >
